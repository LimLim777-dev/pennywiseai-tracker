...
interface SubscriptionDao {
    ...
    /**
     * Active subscriptions the phantom auto-creator materialises: every
     * INCOME-direction row (#371), plus EXPENSE-direction rows the user
     * opted into auto-generation (recurring auto-debits with no
     * SMS/notification), whose next_payment_date is today or earlier —
     * one transaction per missed cycle for each row returned here.
     */
    @Query("""
        SELECT * FROM subscriptions
        WHERE (direction = 'INCOME' OR auto_generate = 1)
          AND state = 'ACTIVE'
          AND next_payment_date IS NOT NULL
          AND next_payment_date <= :date
        ORDER BY next_payment_date ASC
    """)
    suspend fun getDueAutoGeneratedSubscriptions(date: LocalDate): List<SubscriptionEntity>
